...
struct {
	...
	uint16_t motor_pulses[3] = {80, 110, 50};
	uint32_t last_motor_update;
	uint8_t led_state = 0;
	uint16_t led_pulses[3] = {80, 110, 70};
	...
	if (beat_effect.led_state) {
		if (beat_effect.last_led_update < cur_ms) {
			if (beat_effect.led_state >= ARRAY_SIZE(beat_effect.led_pulses)) {
				...
				beat_effect.led_state++;
				set_leds(beat_effect.led_state & 1 ? CRGB::Red : CRGB::Black);
			}
		}
	}
}
